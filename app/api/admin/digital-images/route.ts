...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const marker = `/storage/v1/object/public/digital-service-images/`;
    const idx = url.indexOf(marker);
    if (idx === -1) return NextResponse.json({ ok: true });
    const path = decodeURIComponent(url.slice(idx + marker.length));
    const supabase = getServiceClient();
    const { error } = await supabase.storage.from('digital-service-images').remove([path]);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    return NextResponse.json({ ok: true });
  } catch (e) {
    return NextResponse.json({ error: e instanceof Error ? e.message : 'تعذر حذف الصورة' }, { status: 500 });
  }
}
